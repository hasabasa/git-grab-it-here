...
const Sidebar = ({
  ...
  return <aside className={cn("fixed left-0 top-0 z-40 h-full bg-white shadow-md transition-all duration-300 ease-in-out", isOpen ? "w-64" : "w-16")}>
      ...
      <div className="px-2 py-4">
        <nav className="space-y-2">
          {menuItems.map(item => <Link key={item.path} to={item.path} className={cn("flex items-center rounded-lg px-3 py-2 transition-colors", location.pathname === item.path ? "bg-primary text-primary-foreground" : "hover:bg-gray-100", !isOpen && "justify-center")}>
